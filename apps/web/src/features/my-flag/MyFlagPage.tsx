...
function visitsToSpots(visits: Visit[]): Spot[] {
  return visits.map((visit) => ({
    id: visit.spotId,
    title: visit.spotTitle,
    address: '',
    contentTypeId: 12,
...
export function MyFlagPage() {
  ...
  const handlePurchase = async (skin: Skin) => {
    if (!skin.id) return;
    try { await purchaseFlagSkin(skin.id); await refreshAccount(); setToast(`${skin.name ?? '스킨'}을 구매했습니다.`); }
    catch (error) { setToast(error instanceof Error ? error.message : '스킨 구매에 실패했습니다.'); }
  };
  const handleEquip = async (skin: Skin) => {
    try { await equipFlagSkin(skin.id); await refreshAccount(); setToast(`${skin.name ?? '스킨'}을 장착했습니다.`); }
    catch (error) { setToast(error instanceof Error ? error.message : '스킨 장착에 실패했습니다.'); }
  };
  ...
  return <main className="page my-flag-page">
    <header className="simple-header my-flag-header"><p className="eyebrow"><Flag size={15} /> 마이 플래그</p><h1>{profile?.nickname ? `${profile.nickname}님의 플래그` : '나만의 로컬 지도를 만들어요'}</h1></header>
    ...
    <section className="section-block"><div className="section-heading"><div><p>내가 방문한 장소</p><h2>내 방문 지도</h2></div><span className="result-count">{visits.length}곳</span></div>{visits.length === 0 ? <div className="status-card">아직 방문한 장소가 없습니다.</div> : <MapPreview spots={visitsToSpots(visits)} />}</section>
    ...
    <section className="section-block"><div className="section-heading"><div><p>포인트로 꾸미는 나만의 플래그</p><h2>플래그 스킨</h2></div></div>{skins.length === 0 ? <div className="status-card">등록된 스킨이 없습니다.</div> : <div className="skin-grid">{skins.map((skin) => { const owned = skin.owned === true; const equipped = skin.equipped === true || skin.isEquipped === true || equippedSkinId === skin.id; return <article className={`skin-card${equipped ? ' skin-card--active' : ''}${!owned ? ' skin-card--locked' : ''}`} key={skin.id}>{!owned ? <LockKeyhole size={22} /> : equipped ? <Flag size={22} /> : <Palette size={22} />}<strong>{skin.name ?? skin.id}</strong><small>{equipped ? '장착 됨' : owned ? '보유 중' : `${skin.price ?? 0}P`}</small>{!owned ? <button type="button" className="primary-button" onClick={() => handlePurchase(skin)}>구매하기</button> : !equipped ? <button type="button" className="primary-button" onClick={() => handleEquip(skin)}>장착하기</button> : <button type="button" className="primary-button" disabled>장착 됨</button>}</article>; })}</div>}</section>
