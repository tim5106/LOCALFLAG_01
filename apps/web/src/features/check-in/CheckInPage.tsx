...
export function CheckInPage() {
  ...
  const eligibleStoredSpot = storedSelectedSpot?.geometryType === 'POINT' && storedSelectedSpot.checkInEnabled !== false ? storedSelectedSpot : null;
  const [state, setState] = useState<LocationState>('idle'); const [accuracy, setAccuracy] = useState<number | null>(null); const [position, setPosition] = useState<{ lat: number; lng: number } | null>(null); const [selectedSpot, setSelectedSpot] = useState<Spot | null>(eligibleStoredSpot); const [message, setMessage] = useState(''); const [precheck, setPrecheck] = useState<PrecheckResult | null>(null); const [loading, setLoading] = useState(false); const [modal, setModal] = useState<CheckInModal | null>(null); const [completedIds, setCompletedIds] = useState<number[]>([]);
  ...
  const nearbySpots = (spotsQuery.data?.data ?? []).filter((spot) => spot.geometryType === 'POINT' && spot.checkInEnabled !== false).map((spot) => completedIds.includes(spot.id) ? { ...spot, checkInCompleted: true } : spot);
  const spots = selectedSpot && !nearbySpots.some((spot) => spot.id === selectedSpot.id) ? [selectedSpot, ...nearbySpots] : nearbySpots;
