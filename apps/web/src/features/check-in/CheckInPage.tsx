...
export function CheckInPage() {
  ...
  const requestLocation = () => { if (!navigator.geolocation) { setState('unsupported'); return; } setState('requesting'); navigator.geolocation.getCurrentPosition(({ coords }) => { const nextAccuracy = Math.round(coords.accuracy); setAccuracy(nextAccuracy); setPosition({ lat: coords.latitude, lng: coords.longitude }); setState(nextAccuracy > 50 ? 'inaccurate' : 'measured'); }, (error) => setState(error.code === error.PERMISSION_DENIED ? 'denied' : 'idle'), { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }); };
  useEffect(() => { requestLocation(); }, []);
  useEffect(() => { if (selectedSpot) setPrecheck({ eligible: true, spotId: selectedSpot.id, distanceM: 0, allowedRadiusM: CHECK_IN_RADIUS_METERS, accuracyM: accuracy ?? 0, reasons: [], estimatedReward: 0 }); }, [selectedSpot, accuracy]);
  const handleCheckIn = async () => { if (!position || !selectedSpot || accuracy === null || !precheck?.eligible) return; setLoading(true); const input: PositionInput = { lat: position.lat, lng: position.lng, accuracyM: accuracy, capturedAt: new Date().toISOString() }; try { const precheckRes = await precheckSpot(selectedSpot.id, input); if (!precheckRes.data.eligible) { setMessage(precheckRes.data.reasons?.[0] ?? '현장 인증 조건을 충족하지 못했습니다.'); return; } const checkInRes = await createCheckIn(selectedSpot.id, input); const result: ApiCheckInResult = checkInRes.data; setCompletedIds((ids) => ids.includes(selectedSpot.id) ? ids : [...ids, selectedSpot.id]); setModal({ status: result.status === 'REVIEW' ? 'pending' : 'success', points: result.reward?.points, message: result.status === 'REVIEW' ? '인증 결과를 확인하고 있어요.' : '체크인이 완료되었습니다.' }); } catch (caught) { const err = caught instanceof CheckInApiError ? caught : null; const errorMessage = err?.status === 501 ? '현장 인증 백엔드 기능 준비 중입니다. (테스트는 /check-in/test 페이지를 이용해주세요)' : err?.status === 404 ? '현재 시범 운영 등록 장소가 아닙니다. (SPOT_NOT_FOUND)' : caught instanceof Error ? caught.message : '현장 인증 처리에 실패했습니다.'; setMessage(errorMessage); window.alert(errorMessage); } finally { setLoading(false); } };
  const spots = (spotsQuery.data?.data ?? []).map((spot) => completedIds.includes(spot.id) ? { ...spot, checkInCompleted: true } : spot);
