...
export function SpotCard({ spot, onSelect }: { spot: Spot; onSelect?: (spot: Spot) => void }) {
  ...
  return <article className="spot-card" tabIndex={0} role="button" onClick={() => onSelect?.(spot)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect?.(spot); } }}>
    ...
    <div className="spot-card__content"><div><h3>{spot.title}</h3><p><MapPin size={14} /> {spot.address}</p></div><div className="spot-card__meta">{spot.isDecliningArea && <span>인구감소지역 · 2.5배</span>}{spot.estimatedReward !== undefined && <strong>예상 {spot.estimatedReward}P</strong>}</div></div>
  </article>;
}
