...
interface MapPreviewProps { spots: Spot[]; onSelect?: (spot: Spot) => void; onViewportChange?: (viewport: { minLat: number; minLng: number; maxLat: number; maxLng: number }) => void; }
...
export function MapPreview({ spots, onSelect, onViewportChange }: MapPreviewProps) {
  ...
  useEffect(() => { if (mapState !== 'ready' || !window.kakao || !mapRef.current) return; markersRef.current.forEach((marker) => marker.setMap(null)); markersRef.current = spots.filter((spot) => Number.isFinite(spot.location.lat) && Number.isFinite(spot.location.lng)).map((spot) => { const marker = new window.kakao.maps.Marker({ map: mapRef.current, position: new window.kakao.maps.LatLng(spot.location.lat, spot.location.lng), title: spot.title }); window.kakao.maps.event.addListener(marker, 'click', () => onSelect?.(spot)); return marker; }); return () => markersRef.current.forEach((marker) => marker.setMap(null)); }, [mapState, spots, onSelect]);
