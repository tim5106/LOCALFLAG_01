...
export function getMapSpotState({
  spot,
  selectedSpotId = null,
  nearbySpotId = null,
  nearbySpotIds = new Set<number>(),
  visitedSpotIds = new Set<number>(),
}: MapSpotStateInput): MapSpotState {
  ...
  return {
    ...
    visited,
    checkInAvailable: !visited && !checkInUnavailable && nearby,
    checkInUnavailable,
  };
}
...
export function getNearbySpotIds(spots: readonly NearbySpot[], userLocation: Spot['location']): Set<number> {
  return new Set(spots
    .filter((spot) => distanceInMeters(spot.location, userLocation) <= (spot.checkInRadiusM ?? 100))
    .map((spot) => spot.id));
}
